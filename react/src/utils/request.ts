...
export function checkStatus(response: any) {
  if (response.status >= 200 && response.status < 300) {
    ...
    response.text().then((errortext: any) => {
  ...
  return fetch(url, newOptions)
    ...
    .then((response) => {
      ...
      if (!response.headers.has('X-Total-Count')) {
        if(newOptions.headers.Accept=='text/plain')
        {
          return response.text();
        }
        else
...
export function mboxTaskRequest(url: string, options: any = {}) {
  return task(request(url, options));
}